...
Cone::Cone(const Point3i& pos, const std::vector<int>& sides, const int& texture)
...
void Cone::render() 
{
	...
	for (int h = KS_NUMC; h > 0; --h) {
		for (int i = 0; i <= KS_NUMT; ++i) {
			...
			glTexCoord2f(1.0 * h / KS_NUMC, 1.0 * i / KS_NUMT);
			setPixel(x, y, z);
			...
			glTexCoord2f(1.0 * h / KS_NUMC, 1.0 * (i + 1) / KS_NUMT);
			setPixel(x, y, z);
			...
			glTexCoord2f(1.0 * (h - 1) / KS_NUMC, 1.0 * (i + 1) / KS_NUMT);
			setPixel(x, y, z);
			...
			glTexCoord2f(1.0 * (h - 1) / KS_NUMC, 1.0 * i / KS_NUMT);
			setPixel(x, y, z);
		}
	}
	...
	for (int i = 0; i <= KS_NUMT; ++i) {
		...
		glTexCoord2f(1.0 * i / KS_NUMT, 1.0 * i / KS_NUMT);
		setPixel(x, y, z);
	}
...
const int Cone::KS_NUMC = 32;
const int Cone::KS_NUMT = 64;