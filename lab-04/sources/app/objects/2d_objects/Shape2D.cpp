...
Shape2D::Shape2D(const Point& rect_start, const Point& rect_end, const Color& boundary_colr, const Color& fill_color) {
...
Shape2D::Shape2D(const Shape2D& another) {
...
bool Shape2D::contain(const Point& point) {
...
void Shape2D::boundary_fill() const {
...
void Shape2D::bresenham(Point first, Point last) const
{
	if (first > last)
        swap(first, last);
...
void Shape2D::bresenham_rev(Point first, Point last) const
{
	if (first > last)
        swap(first, last);
...
void Shape2D::initialize(const uint32_t& screen_width, const uint32_t& screen_height) {
    k_borderID.assign(screen_width, std::vector<uint8_t>(screen_height, 0));
}

void Shape2D::clear_buffer() {
    for (auto& x: k_borderID) {
        for (auto& y: x) y = 0;
    }
}

void Shape2D::switchStatus() {

}


void Shape2D::setFillColor(const Color& color)
{
    m_fillColor = color;
}

void Shape2D::setBoundaryColor(const Color& color)
{
    m_boundaryColor = color;
}

Color Shape2D::getFillColor()
{
    return m_fillColor;
}

Color Shape2D::getBoundaryColor()
{
    return m_boundaryColor;
}

void Shape2D::setBoundary(const Point& first, const Point& second) {
    unbound();
...
void Shape2D::setPixel(int x, int y) const {
...
std::stack <uint8_t> Shape2D::k_avalableID;
std::vector<std::vector<uint8_t>> Shape2D::k_borderID;
bool Shape2D::k_hasInititalized = false;